...
function getAllPhotos(req) {
    return new Promise(function(resolve, reject) {
        var userId = req.headers["x-user-id"]
        params = [userId]
        query = 'select face_id as "faceid", url as "url", bounding_box as "boundingbox" from all_photos_by_users where user_id = ?;'

        cassandraDal.executeQuery(query, params)
            .then(function(result) {
                resolve({ "faces": _.uniq(result.rows, 'url') })
            })
            .catch(function(err) {
                var obj = { "errors": "Error while fetching data from cassandra", "code": errorConstants.ERROR_INTERNAL_SERVER_ERROR_FROM_CASSANDRA }
                reject(obj);
